...
impl<V: PaserkVersion> PaserkLocalPw<V> {
    ...
    /// Creates a new `PaserkLocalPw` from raw components.
    #[cfg(any(feature = "k2", feature = "k4"))]
    fn new(
        salt: [u8; ARGON2_SALT_SIZE],
        nonce: [u8; XCHACHA20_NONCE_SIZE],
    ...
    const fn data_size() -> usize {
        ARGON2_SALT_SIZE + XCHACHA20_NONCE_SIZE + 32 + PBKW_TAG_SIZE
    }
}
...
impl<V: PaserkVersion + crate::core::version::UsesArgon2> PaserkLocalPw<V> {
    /// Wraps a symmetric key with a password using PBKW.
    ///
    /// # Arguments
    ///
    ...
    pub fn try_wrap(
        ...
        Ok(Self::new(salt, nonce, ciphertext, tag))
    }
...
impl<V: PaserkVersion> TryFrom<&str> for PaserkLocalPw<V> {
    ...
    fn try_from(paserk: &str) -> Result<Self, Self::Error> {
        ...
        ciphertext.copy_from_slice(&data[offset..offset + 32]);
        offset += 32;
...
mod tests {
    ...
    fn test_wrap_unwrap_roundtrip() {
        ...
        let wrapped = PaserkLocalPw::<K4>::try_wrap(&key, password, test_params())
            .expect("wrap should succeed");
    ...
    fn test_serialize_parse_roundtrip() {
        ...
        let parsed =
            PaserkLocalPw::<K4>::try_from(serialized.as_str()).expect("parse should succeed");
